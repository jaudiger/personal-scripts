...
export def detect-nixpkgs-build-type [content: string]: nothing -> string {
...
export def extract-nixpkgs-build-deps [content: string]: nothing -> list<string> {
    let blocks = try {
        $content
...
export def fetch-nix-file-info [name: string]: nothing -> any {
    ...
    $paths | reduce --fold null {|path, file|
        if $file != null {
            $file
        } else {
...
def fetch-nix-file [name: string]: nothing -> any {
    let file = fetch-nix-file-info $name
    if $file == null { null } else { $file.content }
}

export def fetch-nixpkgs [limit: int, exclusions: list<string>]: nothing -> list<record> {
        ...
        | each {|commit|
            ...
            if $name_match == null {
                ...
                if (is-excluded $name $exclusions) {
                    ...
                    make-package-json $name "nixpkgs" $version $description $repository {nixpkgs: $recipe}
                }
            }
        }
        | compact
    log $"Fetched ($packages | length) packages from Nixpkgs"
    $packages
}
